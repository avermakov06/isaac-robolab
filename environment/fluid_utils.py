...
def generate_inside_point_cloud(sphereDiameter, cloud_points, scale = 1, max_particles = 3000):
    ...
    finalSamples = []
    contains = in_hull(samples, cloud_points * scale)  # must filter points using *scaled* hull
...
def set_particle_system_for_cup(
    ...
    # particle_rest_offset = 0.75 * scale

    cloud_points = np.array(volume_mesh.GetPointsAttr().Get())

    positions_list  = generate_inside_point_cloud(sphereDiameter=particle_rest_offset * (2.0 + 0.08), cloud_points = cloud_points, scale=scale)
    ...
    if enable_iso_surface:
        ...
        water_url = (Path(__file__).parents[1] / "assets" / "Water.mdl").as_posix()
    ...
    if mug_init_rotation is not None:
        particles.AddRotateXOp().Set(mug_init_rotation)
